...

{
    public class controller_AccesoSistema
    {
        ...
        //Funcion para poder usar el sqlconnection
        ...
        public UsuarioBE Get_Acceder_Sistema(string Usuario, string Password, String rucEmpresa)
        {
            try
            {
                ...
                {
                    SqlCommand command = new SqlCommand("gnvalidarUsuario", connection);
                    command.CommandType = CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("@usuario", Usuario);
                    connection.Open();
                    object result = command.ExecuteScalar();
                    // Si el resultado no es nulo, asigna el valor a la variable contrasena
                    if (result != null)
                    {
                        contrasenia = result.ToString();
                    }  
                }
                string ValidaClave = Encryptar.GetMD5(Password);
                if (ValidaClave != contrasenia)
                {
                    return null;
                }
                else
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(sqlConnectionString))
                    {
                        ...
                        try
                        {
                            ...
                            retorno = Int32.Parse(command.Parameters["@vo_retorno"].Value.ToString());
                            retorno_msg = command.Parameters["@vo_retorno_msg"].Value.ToString();
                        }
                        catch (Exception)
                        {
